  ...
  <div>
    ...
    <q-card class="invoice-card">
      ...
      <q-card-actions align="right">
        ...
        <q-btn :color="invoice.status === 'bezahlt' ? 'grey' : 'green'"
          :label="invoice.status === 'bezahlt' ? 'Bereits bezahlt' : 'Als bezahlt markieren'"
          :disable="invoice.status === 'bezahlt'" @click="markAsPaid" />
      </q-card-actions>
...
async function markAsPaid() {
  ...
  try {
    const response = await api.put(`/invoices/${id}`, { status: "bezahlt" });
